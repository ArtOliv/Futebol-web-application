    ...
    for row in rows:
        ...
        if jogador_key not in time_data["jogadores_dict"]:
            time_data["jogadores_dict"][jogador_key] = {
                ...
                "n_altura": float(row["n_altura"]) if isinstance(row["n_altura"], Decimal) else row["n_altura"],
                "n_peso": float(row["n_peso"]) if isinstance(row["n_peso"], Decimal) else row["n_peso"],
                "cartoes": []
            }
